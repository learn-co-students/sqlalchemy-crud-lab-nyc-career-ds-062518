...
def query_all_states():
    Session = sessionmaker(bind=engine)
    session = Session()
    state_query = session.query(State).all()
    return state_query
...
def update_cali():
    ...
    california.population = 50000000
    session.commit()
...
def delete_connecticut():
    Session = sessionmaker(bind=engine)
    session = Session()
    connecticut = session.query(State).filter_by(name = 'Connecticut').first()
    session.delete(connecticut)
    session.commit()
