...
def create_new_york():
    ...
    session = Session()
    new_york = State(name = 'New York', capital_city = 'Albany', population =20000000, landlocked =0)
...
def create_wyoming():
    Session = sessionmaker(bind=engine)
    session = Session()
    wyoming = State(name='Wyoming', capital_city='Cheyenne', population=579315, landlocked=1)
    session.add(wyoming)
    session.commit()
